...
func NewREST(ctx context.Context, scheme *runtime.Scheme, optsGetter generic.RESTOptionsGetter, client client.Client, storeConfig *store.Config) (rest.Storage, error) {
	scheme.AddFieldLabelConversionFunc(
		...
		configv1alpha1.ConvertConfigSetFieldSelector,
	)
	...
	case store.StorageType_File:
		configStore, err = store.CreateFileStore(ctx, scheme, &configv1alpha1.ConfigSet{}, storeConfig.Prefix)
		if err != nil {
			return nil, err
		}
	case store.StorageType_KV:
		configStore, err = store.CreateKVStore(ctx, storeConfig.DB, scheme, &configv1alpha1.ConfigSet{})
		if err != nil {
			return nil, err
		}
	default:
	...
	store := &registry.Store{
		...
		SingularQualifiedResource: configv1alpha1.Resource(configv1alpha1.ConfigSetSingular),
		...
		TableConvertor: NewTableConvertor(configv1alpha1.Resource(configv1alpha1.ConfigSetPlural)),
	}
