...
type drIPRangeEventHandler struct {
	client client.Client
}

// Create enqueues a request for all ip allocation within the ipam
func (r *drIPRangeEventHandler) Create(evt event.CreateEvent, q workqueue.RateLimitingInterface) {
	r.add(evt.Object, q)
}

// Create enqueues a request for all ip allocation within the ipam
func (r *drIPRangeEventHandler) Update(evt event.UpdateEvent, q workqueue.RateLimitingInterface) {
	r.add(evt.ObjectOld, q)
	r.add(evt.ObjectNew, q)
}

// Create enqueues a request for all ip allocation within the ipam
func (r *drIPRangeEventHandler) Delete(evt event.DeleteEvent, q workqueue.RateLimitingInterface) {
	r.add(evt.Object, q)
}

// Create enqueues a request for all ip allocation within the ipam
func (r *drIPRangeEventHandler) Generic(evt event.GenericEvent, q workqueue.RateLimitingInterface) {
	r.add(evt.Object, q)
}

func (r *drIPRangeEventHandler) add(obj runtime.Object, queue adder) {
	cr, ok := obj.(*invv1alpha1.DiscoveryRuleIPRange)
	if !ok {
		return
	}
	...
	for _, dr := range drs.Items {
		if dr.Spec.DiscoveryRuleRef.Name == cr.GetName() &&
			...
			dr.Spec.DiscoveryRuleRef.Kind == invv1alpha1.DiscoveryRuleIPRangeKind {
