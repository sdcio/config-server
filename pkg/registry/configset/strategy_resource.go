...
func (r *strategy) Validate(ctx context.Context, obj runtime.Object) field.ErrorList {
	_, ok := obj.(*config.ConfigSet)
	if !ok {
		var allErrs field.ErrorList
		allErrs = append(allErrs, field.Invalid(
			field.NewPath(""),
			obj,
			fmt.Sprintf("unexpected object, expected: %s, got: %s", config.ConfigSetKind, reflect.TypeOf(obj).Name()),
		))
		return allErrs
	}
	return obj.(*config.ConfigSet).ValidateCreate(ctx)
}
...
func (r *strategy) PrepareForUpdate(ctx context.Context, obj, old runtime.Object) {
	newObj := obj.(*config.ConfigSet)
	oldObj := old.(*config.ConfigSet)
...
func (r *strategy) ValidateUpdate(ctx context.Context, obj, old runtime.Object) field.ErrorList {
	...
	return obj.(*config.ConfigSet).ValidateUpdate(ctx, old)
}
...
func (r *strategy) Update(ctx context.Context, key types.NamespacedName, obj, old runtime.Object, dryrun bool) (runtime.Object, error) {
	...
	newConfig, ok := obj.(*config.ConfigSet)
	if !ok {
		return obj, fmt.Errorf("unexpected new object, expecting: %s, got: %s", r.obj.GetObjectKind().GroupVersionKind().Kind, reflect.TypeOf(obj))
	}
	oldConfig, ok := old.(*config.ConfigSet)
	if !ok {
		return obj, fmt.Errorf("unexpected old object, expecting: %s, got: %s", r.obj.GetObjectKind().GroupVersionKind().Kind, reflect.TypeOf(obj))
	}
